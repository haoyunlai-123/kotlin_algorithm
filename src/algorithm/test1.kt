...
fun countOdds(
    ...
    if (low % 2 != high % 2) {
        return (high - low + 1) shr 1
    } else {
        if (low % 2 == 1) {
            return ((high - low + 1) shr 1) + 1
        } else {
            return (high - low + 1) shr 1
        }
    }
...
fun minDeletion1(nums: IntArray): Int =
    nums.size -
        mutableListOf<Int>()
            .apply {
                var count = 0
...
fun maximumGain(
    ...
    x: Int,
    y: Int,
): Int = if (x >= y) delete1(s, x, y) else delete2(s, x, y)
...
tailrec fun gcd(
    a: Int,
    b: Int,
): Long = if (b == 0) abs(a).toLong() else gcd(b, a % b)

// 最小公倍数
fun lcm(
    a: Int,
    b: Int,
): Long = abs(a.toLong() * b) / gcd(a, b)
...
fun minAddToMakeValid1(s: String): Int =
    s
        .fold(0 to 0) { (score, ans), ch ->
...
fun dailyTemperatures(temperatures: IntArray): IntArray =
    IntArray(temperatures.size).apply arr@{
...
fun nextGreaterElement(
    ...
    IntArray(nums1.size) { -1 }.apply outer@{
        val hash =
...
fun nextGreaterElements(nums: IntArray): IntArray =
    IntArray(nums.size) { -1 }.apply outer@{
        ArrayDeque<Int>().apply {
...
fun largestRectangleArea(heights: IntArray): Int {
    val left =
        IntArray(heights.size).apply out@{
            ArrayDeque<Int>().apply {
...
fun operate(
    staOpe: ArrayDeque<Char>,
    staNum: ArrayDeque<Int>,
) {
