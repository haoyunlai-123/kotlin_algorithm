...
class test2 {
    ...
    fun predictPartyVictory(senate: String): String {
        ...
        while (queR.isNotEmpty() && queD.isNotEmpty()) {
            val a = queR.removeFirst()
            val b = queD.removeFirst()
            if (a < b) {
                queR.addLast(a + senate.length)
            } else {
                queD.addLast(b + senate.length)
            }
        }
...
fun minGroups1(intervals: Array<IntArray>): Int =
    PriorityQueue<Int>(compareBy { it })
        .apply {
...
fun reorganizeString1(s: String): String =
    buildString {
        ...
        val heap =
            PriorityQueue<Char>(compareByDescending { fre[it] }).apply {
                addAll(fre.keys)
            }
...
fun getIntersectionNode(
    ...
    when {
        a >= b -> {
            cur = headA
            diff = a - b
            repeat(diff) { curA = curA?.next }
        }
        else -> {
            cur = headB
            diff = b - a
            repeat(diff) { curB = curB?.next }
        }
    }
    while (curA != curB) {
        curA = curA?.next
...
fun mergeKLists1(lists: Array<ListNode?>): ListNode? = merge(lists, 0, lists.size)
...
fun dailyTemperatures1(temperatures: IntArray): IntArray =
    IntArray(temperatures.size).apply outer@{
